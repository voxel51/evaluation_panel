...
class EvaluationPanel(foo.Panel):
    ...
    def render(self, ctx):
        ...
        plots_container = types.GridView(gap=0.25, align_x="left", align_y="center")
        md_container = types.GridView(gap=15, align_x="center", align_y="center")
        ...
        if ctx.panel.state.my_stack != None:
            if ctx.panel.state.my_stack["menu"]["actions"]["eval_key"] != None:

                # Remove active eval key from the compare list
                compare_keys = eval_keys.values()
                index = compare_keys.index(ctx.panel.state.my_stack["menu"]["actions"]["eval_key"] )
                compare_keys.pop(index)

                # Find the type of eval
                current_eval = ctx.dataset.get_evaluation_info(ctx.panel.state.my_stack["menu"]["actions"]["eval_key"])
        ...
        if ctx.panel.state.my_stack != None:
            if ctx.panel.state.my_stack["tabs"] == 'results':
                ...
                if eval_type and eval_type == "detection":
                    ...
                    panel.list('mAP_evaluations', element_type=mAP_obj, view=mAP_table_view)
                
            elif ctx.panel.state.my_stack["tabs"] == 'info':
                if current_eval:

                    stack.md(f"{current_eval}")

                    """
                    if eval_type == "detection":
                        # Define table structure
                        info_obj = types.Object()
                        info_obj.str('key')
                        info_obj.str('version')
                        info_obj.str('timestamp')
                        info_obj.str('cls')
                        info_obj.str('type')
                        info_obj.str('method')
                        info_obj.str('pred_field')
                        info_obj.str('gt_field')
                        info_obj.float('iou')
                        info_obj.bool('classwise')
                        info_obj.str('iscrowd')
                        info_obj.bool('use_masks')
                        info_obj.bool('use_boxes')
                        info_obj.bool('compute_mAP')
                        info_obj.list('iou_threshs')
                        info_obj.int('max_preds')
                        info_obj.int('error_level')


                        # Define the table
                        info_view = types.TableView() 
                        info_view.add_column('key', label="Class")
                        info_view.add_column('version', label="Precision")
                        info_view.add_column('timestamp', label="Recall")
                        info_view.add_column('cls', label="F1-Score")
                        info_view.add_column('type', label="Support")
                        info_view.add_column('method', label="Support")
                        info_view.add_column('pred_field', label="Support")
                        info_view.add_column('gt_field', label="Support")
                        info_view.add_column('iou', label="Support")
                        info_view.add_column('classwise', label="Support")
                        info_view.add_column('iscrowd', label="Support")
                        info_view.add_column('use_masks', label="Support")
                        info_view.add_column('use_boxes', label="Support")
                        info_view.add_column('compute_mAP', label="Support")
                        info_view.add_column('max_preds', label="Support")
                        info_view.add_column('error_level', label="Support")

                        # Add table to the stack
                        panel.list('eval_info', element_type=info_obj, view=info_view)
                    """
    ...
    def on_change_config(self, ctx):
        current_eval = ctx.dataset.get_evaluation_info(ctx.panel.state.my_stack["menu"]["actions"]["eval_key"])
        eval_type = current_eval.config.type
        results = ctx.dataset.load_evaluation_results(ctx.panel.state.my_stack["menu"]["actions"]["eval_key"])
        ...
        #ctx.panel.state.my_stack["mAP_evaluations"] = table_list
        ctx.panel.state.evaluations = table_list
        ...
        if eval_type == "detection":
            ...
            mAP_list.append(new_row)
            #ctx.panel.state.my_stack["mAP_evaluations"] = mAP_list
            ctx.panel.state.mAP_evaluations = mAP_list
